...
class PrintQueueOrchestrator(
    ...
    fun retryPrint(jobId: String): PrintJobEntity? {
        val current = dao.getById(jobId) ?: return null
        if (current.state != PrintJobState.NEEDS_ATTENTION) {
            return current
        }

        val moved = dao.updateState(
            jobId = jobId,
            expectedCurrent = PrintJobState.NEEDS_ATTENTION,
            newState = PrintJobState.QUEUED,
            attemptCount = 0,
            errorCode = null,
            errorMessage = null,
